...
1.  Fill in the blank: A streak length of 1 means one <em>hit</em> followed by one miss.


2.  Fill in the blank: A streak length of 0 means one <em>miss</em> which must occur after a 
...
# type your code for the Exercise here, and Knit
notHotHandSteak <- calc_streak(sim_basket)
...
```{r plot-sim-streaks}
# type your code for the Exercise here, and Knit
ggplot(data=notHotHandSteak , aes(x = length)) +
  geom_histogram(binwidth = 1)
