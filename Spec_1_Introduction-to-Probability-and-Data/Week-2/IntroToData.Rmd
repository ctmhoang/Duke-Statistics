...
sfo_feb_flights %>%
  summarise(mean_ad = mean(arr_delay), std_ad = sd(arr_delay),median_ad = median(arr_delay), n = n())
```
...
nycflights %>% summarise(percent_OnTime = sum(arr_type == "on time") / n())
nycflights %>% summarise( prop = sum(dep_type == "delayed") /sum(arr_type == "delayed"& dep_type =="on time"))
```
